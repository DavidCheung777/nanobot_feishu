...
通过 Python 调用 `FeishuClient` 方法：

```python
from feishu import FeishuClient

client = FeishuClient()
client.create_bitable_record("app_token", "table_id", {"字段": "值"})
```

## 🚨 执行前必读

- ✅ **创建数据表**：支持两种模式 — ① 明确需求时，在 `create` 时通过 `table.fields` 一次性定义字段（减少 API 调用）；② 探索式场景时，使用默认表 + 逐步修改字段（更稳定，易调整）
- ⚠️ **默认表的空行坑**：`app.create` 自带的默认表中会有空记录（空行）！插入数据前建议先调用 `list_bitable_records` + `batch_delete_bitable_records` 删除空行，避免数据污染
- ✅ **写记录前**：先调用 `list_bitable_fields` 获取字段 type/ui_type
...
以下 JSON 示例用于说明参数结构，实际调用请使用上表对应的 Python 方法。

| 用户意图 | 工具 | action | 必填参数 | 常用可选 |
|---------|------|--------|---------|---------|
| 查表有哪些字段 | `list_bitable_fields` | app_token, table_id | - |
| 查记录 | `list_bitable_records` | app_token, table_id | filter, sort, field_names |
| 新增一行 | `create_bitable_record` | app_token, table_id, fields | - |
| 批量导入 | `batch_create_bitable_records` | app_token, table_id, records (≤500) | - |
| 更新一行 | `update_bitable_record` | app_token, table_id, record_id, fields | - |
| 批量更新 | `batch_update_bitable_records` | app_token, table_id, records (≤500) | - |
| 批量删除 | `batch_delete_bitable_records` | app_token, table_id, record_ids | - |
| 创建多维表格 | `create_bitable_app` | name | folder_token |
| 创建数据表 | `create_bitable_table` | app_token, name | fields |
| 创建字段 | `create_bitable_field` | app_token, table_id, field_name, type | property |
| 创建视图 | `create_bitable_view` | app_token, table_id, view_name, view_type | - |
...
**强制流程**：
1. 先调用 `list_bitable_fields` 获取字段的 `type` 和 `ui_type`
