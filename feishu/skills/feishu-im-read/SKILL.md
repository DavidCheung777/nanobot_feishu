...
通过 Python 调用 `FeishuClient` 用户态方法：

```python
from feishu import FeishuClient

client = FeishuClient()
client.user_get_messages(chat_id="oc_xxx", relative_time="last_24_hours", user_access_token="u-xxx")
```

## 执行前必读

- 该 Skill 中的所有消息读取工具均以用户身份调用，只能读取用户有权限的会话
- `user_get_messages` 中 `open_id` 和 `chat_id` 必须二选一
- 消息中出现 `thread_id` 时，根据用户意图判断是否用 `user_get_thread_messages` 读取话题内回复
- 以用户身份读取后，如果消息内容中出现资源标记时，用 `user_fetch_resource` 下载，需要 `message_id` + `file_key` + `type`
...
| 用户意图 | 工具 | 必填参数 | 常用可选 |
|---------|------|---------|---------|
| 获取群聊/单聊历史消息 | `user_get_messages` | chat_id 或 open_id（二选一） | relative_time, start_time/end_time, page_size, sort_rule |
| 获取话题内回复消息 | `user_get_thread_messages` | thread_id（omt_xxx） | page_size, sort_rule |
| 跨会话搜索消息 | `user_search_messages` | 至少一个过滤条件 | query, sender_ids, chat_id, relative_time, start_time/end_time, page_size |
| 下载消息中的图片 | `user_fetch_resource` | message_id, file_key（img_xxx）, type="image" | - |
| 下载消息中的文件/音频/视频 | `user_fetch_resource` | message_id, file_key（file_xxx）, type="file" | - |
...
| 场景 | 行为 |
|------|------|
| 获取历史消息并需要理解上下文（默认） | 对发现的 thread_id 调用 `user_get_thread_messages` 获取最新 10 条回复 |
...
`user_search_messages` 支持跨所有会话搜索消息：
...
消息内容中可能出现以下资源标记，用 `user_fetch_resource` 下载：
...
`user_get_messages` 和 `user_search_messages` 支持时间过滤，话题消息不支持。
