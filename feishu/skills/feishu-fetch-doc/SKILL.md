...
通过 Python 调用 `FeishuClient.fetch_doc_markdown` 获取云文档 Markdown 内容。

```python
from feishu import FeishuClient

client = FeishuClient()
doc = client.fetch_doc_markdown("https://xxx.feishu.cn/docx/doxcnXXXX")
```

## 重要：图片、文件、画板的处理

**文档中的图片、文件、画板需要通过 `FeishuClient.download_doc_media` 单独获取！**
...
2. 调用 `download_doc_media` 下载：
   ```python
   client.download_doc_media(resource_token="提取的token", resource_type="media", output_path="/path/to/save/file")
   ```
...
1. **先调用 `client.get_wiki_node` 解析 wiki token**：
   ```python
   client.get_wiki_node("wiki_token_here")
   ```
...
| obj_type | 工具 | 传参 |
|----------|------|------|
| `docx` | `fetch_doc_markdown` | doc_id = obj_token |
...
3. 调用 `fetch_doc_markdown`（doc_id: doxcnXYZ789）

## 工具组合

| 需求 | 工具 |
|------|------|
| 获取文档文本 | `fetch_doc_markdown` |
| 下载图片/文件/画板 | `download_doc_media` |
| 解析 wiki token 类型 | `get_wiki_node` |
| 读写电子表格 | `feishu_sheet` |
