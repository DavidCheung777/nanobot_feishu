...
|---------|------|--------|---------|---------|---------|
| 创建会议 | `create_calendar_event` | summary, start_time, end_time | user_open_id | attendees, description, location |
| 查某时间段日程 | `list_calendar_events` | start_time, end_time | - | - |
| 改日程时间 | `update_calendar_event` | event_id, start_time/end_time | - | summary, description |
| 搜关键词找会 | `search_calendar_events` | query | - | - |
| 回复邀请 | `reply_calendar_event` | event_id, rsvp_status | - | - |
| 查重复日程实例 | `list_calendar_event_instances` | event_id, start_time, end_time | - | - |
| 查忙闲 | `list_calendar_freebusy` | time_min, time_max, user_ids[] | - | - |
| 邀请参会人 | `create_calendar_event_attendees` | calendar_id, event_id, attendees[] | - | - |
...
**查询预约结果**：使用 `list_calendar_event_attendees` 查看 `rsvp_status`。
...
以下 JSON 示例用于说明参数结构，实际调用请使用上文列出的 Python 方法。
